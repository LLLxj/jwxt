<template>
  <div id="J_offprice_echarts_teacher" class="chart-box"></div>
</template>

<script>
  import { offPriceDay, offPriceWeek, offPriceMonth } from '@/api/teacherLog'
  export default {
    props: {
    ...
    methods: {
      ...
      init () {
        ...
        this.echartsLine = echarts.init(document.getElementById('J_offprice_echarts_teacher'), 'walden')
        // 加载数据
        this.initData()
      },
      // 初始化数据
      ...
      initParams () {
        switch (parseInt(this.value)) {
          case 1:
            this.loadApi = offPriceDay
            this.showData = this.dayData
            this.valueName = '日'
            break
          case 2:
            this.loadApi = offPriceWeek
            this.showData = this.weekData
            this.valueName = '周'
            break
          case 3:
            this.loadApi = offPriceMonth
