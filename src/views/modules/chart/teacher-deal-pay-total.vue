<template>
  <div id="J_common_echarts_teacher" class="chart-box"></div>
</template>

<script>
  import { payTotalDay, payTotalWeek, payTotalMonth } from '@/api/teacherLog'
  export default {
    props: {
    ...
    methods: {
      ...
      init () {
        ...
        this.echartsLine = echarts.init(document.getElementById('J_common_echarts_teacher'), 'walden')
        // 加载数据
        this.initData()
      },
      // 初始化数据
      ...
      initParams () {
        switch (parseInt(this.value)) {
          case 1:
            this.loadApi = payTotalDay
            this.showData = this.dayData
            this.valueName = '日'
            break
          case 2:
            this.loadApi = payTotalWeek
            this.showData = this.weekData
            this.valueName = '周'
            break
          case 3:
            this.loadApi = payTotalMonth
      ...
      loadDataFun () {
        ...
        _this.showData = {}
        // 从服务端请求数据
        this.loadApi(this.loadNum).then(res => {
